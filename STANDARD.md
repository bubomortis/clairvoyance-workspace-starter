...
**Version:** 1.11 · **Status:** Free to copy, adapt, and share.
...
🔑 **The limit that matters most, stated plainly because it constrains everything below.** As of
this revision, **no controlled study has shown that a *generic* repository context file moves task
correctness in either direction.** The strongest evidence is a two-agent ablation over 288
evaluated runs with hidden gold-test scoring, which found no measurable correctness effect for
either agent and — in a pre-registered probe — that a real, good-quality context file **never
converted a near-miss failure into a pass**. Its failure analysis says why: agents lose on
implementation skill, not on repository knowledge a document could supply.

**Read the qualifier as load-bearing.** That study varied *how* context was delivered, not whether it
had been tuned — in the authors' words, *"the injection channel rather than content."* Tuning is
therefore untouched by it, which is why **C1a stands unchanged** a hundred lines below: guidance
tuned against observed failures does beat no file at all, significantly, and that finding is not in
tension with this one.

**And state the bound as precisely as the authors do, because it is a strength and not a hedge.** The
null is descriptive rather than powered: three Python repositories, a minimum detectable effect above
**30pp**, and a 10pp effect that would not have been visible at all. The authors decline to call it a
*powered* equivalence claim, note that their always-on condition is *stronger* than the natural
workflow so
the generalisation is an inference rather than a measurement, and explicitly **cannot rule out
benefits for purpose-built or task-specific context**. The honest band is 10–15pp, and narrowing it
would take roughly 120 tasks. Quoting the null without that is the same overread this document
corrected in C1a at v1.10.

**What is measured is process and adherence**, not accuracy: fewer wasted actions, lower latency and
token cost, better compliance with stated constraints. That is a real and sufficient reason to write
these documents well. It is not a claim that they make the work correct, and this Standard does not
make one. **C1a survives intact** — tuning is still the only intervention measured to significantly
beat having no file — because tuning is precisely what that ablation did not test.

The honest one-line version, unchanged since it was first written here: *context files reduce
wandering without improving the destination.*
...
**How common is the untuned file? Measured: about a quarter.** A survey of 100 popular open-source
repositories found **24% whose agent instruction file had exactly one commit** — generated by an
`/init`-style command and never revised since. That study checked the obvious objection and
dismissed it: none of those projects were dormant, and two had over 1,500 commits *after* the file
was created. The file was not stale because the project stopped; it was stale because nobody went
back to it.

*This is the population C1a addresses, and it is not an edge case.*
...
🔑 **The condition is the load-bearing half, and a bare path is a defect.** A repository-mining study
of 100 projects names this a *blind reference* — a path cited without purpose or scope — and reports
the practitioner consequence bluntly: mention a path and the agent will often ignore it; you have to
tell it **why and when** to read the file. A separate corpus study of 34 governance files found the
mirror failure: repositories where the governance content exists but **not in the location agents
are instructed to read**, including one route pointing at a file that returns 404 with nothing
detecting it. *So the `IF <situation> →` form is not decoration. `- see docs/auth.md` is the smell;
the condition is the remedy.*

⚠️ **A route only routes if it can be found, and adding one does not retroactively deliver it.**
Two failure modes, both local findings, neither covered by the studies above:

- **A router in a hidden directory is invisible to the default search.** Measured here: a
  dot-prefixed directory returned **zero** matches under a plain recursive search and one match only
  when hidden files were explicitly included. Every agent that *searched* instead of following the
  router concluded, correctly for its tools, that the rule did not exist.
- **A route added mid-session never reaches an agent that already loaded the index.** Also measured:
  one agent's read returned lines 1–49 of a router, stopping just short of a route appended at line
  51 minutes earlier. It had no reason to re-read.

**Site the router where an ordinary search will find it, and treat a new route as reaching future
sessions only.** If a rule must apply *now*, deliver the rule text, not a pointer to it.
...
**Two concrete ways the budget is wasted, both measured across 100 repositories:**

🔑 **Do not spend instruction budget on what a deterministic tool already enforces.** Rules restating
what a linter or formatter checks — naming conventions, indentation, import ordering, line length —
were the **single most common defect found, in 62% of files**. The practitioner framing is the right
one: these are deterministic problems with deterministic solutions, handled faster and with perfect
consistency by a pre-commit hook, and every line spent on them competes with the architectural
constraint that actually needs the model's attention.

**Length has a published ceiling and a large minority of files exceed it.** The vendor guidance for
one major runtime is *under 200 lines per file; longer files consume more context and reduce
adherence* — **42% of surveyed files were at or over 200 lines**, the largest at 1,477. Treat 200 as
a review trigger, not a hard limit.

⚠️ **Adherence is not correctness.** These figures measure prevalence, and the vendor guidance
speaks to adherence. **No controlled study has shown a *generic* context file moving task
correctness in either direction** — see §0, and note that tuned guidance is a separate case with its
own evidence at C1a. Trim for attention and cost, and do not promise accuracy you cannot demonstrate.
...
per-agent memory is not a decision record.

⚠️ **The reason is not the obvious one, and getting it wrong is worse than not stating it.** Earlier
versions of this criterion said per-agent memory is *invisible to every other agent*. **Measured on
one install, the opposite was true:** the memory store was keyed to the working directory, not to the
agent — **25 agents across 5 stores, 7 of them sharing a single store** — so agents started in the
same directory silently read and wrote the same files. It surfaced only when a reviewer's ruling was
found inside an engineer's memory file, and only by comparing modification times. Nothing announced
it.

**That is one runtime, observed once; whether yours behaves this way is worth ten seconds of
checking.** The criterion does not rest on it. Both readings fail the same test — a store that is
*not reliably private and not reliably shared*, with no author on any entry, cannot serve as a
decision record.

Two consequences follow for anyone whose runtime behaves this way. **Attribute in the text, not in
a header** — a first-person pronoun copied between agents re-points at whoever reads it, and an
instruction can invert its meaning in transit. And **never blind-write a memory file**: where the
tooling offers a read-then-edit path whose failure is *loud*, that failure is often the only
...
10. **Resolve paths through one resolver.** Absolute paths baked into *state files* at creation are
    ...
    scripts.
11. **Match the model to the instruction *structure* of the seat, not to a leaderboard — and be
    strictest about it on unattended paths.** Two independent teams found local-model Staff
    completing inference but failing to deliver the completion handoff, so **keep local-model Staff
    off unattended paths** unless you have verified end-to-end delivery yourself. But hosting is the
    narrower half of the rule; capability against *nesting* is the load-bearing half.

    🔑 **The cliff is at the first level of nesting, and it is universal.** A 600-prompt benchmark
    with deterministic checkers at every scope found **every** model losing **37–53 absolute points**
    between a flat constraint list and one level of nesting — including the two strongest, which fell
    from 86.7% to 49.3% and 78.7% to 42.0%. Flat instruction-following is nearly saturated; nested is
    not. And separation *widens* with depth: at three levels the top two retained 35.3% and 22.7%
    while five others collapsed to 6% or below. **Model choice matters far more for a seat with
    nested conditionals than for one with a flat checklist.**

    This is not a hypothetical shape. In a corpus of 1,232 real production prompts, **67.8% of
    format-bearing prompts carried at least one nested scope**, and constraint specifications
    occupied **46.4% of prompt text** on average.

    ⚠️ **The corollary is the part people get backwards.** The reflex after an unattended failure is
    to add a rule. Every conditional you add deepens the tree the model already could not hold — so
    the fix that feels safest makes the next failure more likely. **Flatten instead:** move whatever
    is mechanically decidable into code, and leave the seat the residue. A seat that needs no
    judgement barely needs a model.

    ⚠️ **Overall benchmark scores hide this.** Across 11 models on an enterprise instruction-following
    suite, no model exceeded 75% overall, cross-category spread reached 25.5 points, and rankings
    reshuffled completely between categories — one model placing first on format compliance sat fifth
    or sixth on ranking and on appropriate refusal. **Select against the capability the seat actually
    exercises.**
...
| R31 | Model capability is matched to the seat's instruction structure; local-model Staff are not on unattended paths | All |
